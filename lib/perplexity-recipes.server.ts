  ...
  const courses = (parsed.courses ?? []).flatMap((candidate): MenuCourse[] => {
    ...
    return [{
      ...
      instructions: {
        ...
        rightsNote: "Perplexity discovery stores original metadata and a provider-scaled ingredient list only. Follow the linked publisher for authoritative instructions.",
        attribution: "Recipe discovered through Perplexity Agent API web search.",
      },
      quantityScaling: {
        status: "PROVIDER_SCALED",
        sourceServings: input.servings,
        targetServings: input.servings,
        factor: 1,
        note: "Perplexity returned quantities already scaled for the requested servings; host verification is required.",
      },
      prepMinutes,
      cookMinutes,
      dietaryTags: tags,
